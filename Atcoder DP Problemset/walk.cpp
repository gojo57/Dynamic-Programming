...
vvi power(vvi &adj, int k){
    if(k == 1) return adj;
    vvi temp = power(adj, k/2);
    vvi ans(adj.size(), vi(adj.size()));
    ...
    if(k%2 == 1){
        vvi temp2(adj.size(), vi(adj.size()));
        for(int i = 0; i<adj.size(); i++){
            for(int j = 0; j<adj.size(); j++){
                for(int k = 0; k<adj.size(); k++){
                    temp2[i][j] += ans[i][k]*adj[k][j];
                    temp2[i][j] %= mod;
                }
            }
        }
...
int solve(int k, vvi &adj){
    vvi ak = power(adj, k); // Ak[i][j] = number of paths of length k from i to j, Ak = Ak-1*Adj
    int ans = 0;
