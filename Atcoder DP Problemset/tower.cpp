...
// dp(i, j) = maximum pass value such that subset of [1-i] and stack the boxes such that the tower can withstand a weight of atleast j on it.

int solve(vector<box> &v){
    int n = v.size();
    ...
    for(int i = 1; i<n; i++){
        for(int j = 0; j<=10000; j++){
            int op1 = dp[i-1][j];
            int op2 = (v[i].si>=j) ? (v[i].val+(j+v[i].wi>10000?0:dp[i-1][j+v[i].wi])) : 0;
            dp[i][j] = max(op1, op2);
        }
    }
    return dp[n-1][0];
...
signed main()
{
    ...
    sort(all(v), [&](box a, box b){
        return a.si+a.wi > b.si+b.wi;
    });
    cout<<solve(v)<<"\n";
