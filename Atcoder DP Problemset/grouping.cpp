...
void prepareSums(int mask, int n, vvi &a){
    for(int i = 1; i < (1<<n); i++) dp[i] = inf;
    for(int i=0;i<(1<<n);i++){
        sums[i] = calculate(i, a);
    }
}

int solve(int mask, int n, vvi &a){
    if(mask == 0) return 0;
    if(dp[mask] != inf) return dp[mask];
    int ans = 0;
    for(int submask = mask; submask>0; submask = (submask-1)&mask){
        int sum = sums[submask];
        int rem = mask^submask;
        ans = max(ans, sum+solve(rem, n, a));
    }
    return dp[mask] = ans;
}
...
signed main()
{
    ...
    prepareSums((1<<n)-1, n, aa);
    cout<<solve((1<<n)-1, n, aa)<<endl;
