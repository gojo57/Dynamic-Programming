...
int solve(vector<int> &v, vector<vector<int>> &sum, vector<vector<int>> &dp, int i, int j){
    if(i == j) return 0;
    if(dp[i][j] != -1) return dp[i][j];
    int ans = LLONG_MAX;
    for(int k = i; k<j; k++){
        ans = min(ans, solve(v, sum, dp, i, k) + solve(v, sum, dp, k+1, j) + sum[i][j]);
    }
    return dp[i][j] = ans;
}
...
signed main()
{
    ...
    vvi dp(n, vi(n, -1)), sum(n, vi(n, 0));
    for(int i = 0; i<n; i++){
        for(int j = i; j<n; j++){
            if(i == j) sum[i][j] = v[j];
            else sum[i][j] = sum[i][j-1] + v[j];
        }
    }
    cout<<solve(v, sum, dp, 0, n-1)<<endl;
