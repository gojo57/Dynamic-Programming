...
signed main()
{
    ...
    while(tst--){
        ...
        for(int i = 0; i<n; i++){
            for(int j = 0; j<n; j++){
                if(i == 0 && j == 0) pref[i][j] = dp[i][j];
                else if(i == 0) pref[i][j] = dp[i][j] + pref[i][j-1];
                else if(j == 0) pref[i][j] = dp[i][j] + pref[i-1][j];
                else pref[i][j] = dp[i][j] + pref[i-1][j] + pref[i][j-1] - pref[i-1][j-1];
            }
        }
        int ans = -1;
        for(int i = 1; i<n-1; i++){
            for(int j = 1; j<n-1; j++){
                ...
                int y = pref[i][n-1]-x;
                int z = pref[n-1][j]-x;
                int w = pref[n-1][n-1]-x-y-z;
                ans = max(ans, min({x, y, z, w}));
            }
        }
        cout<<ans<<"\n";
    }
