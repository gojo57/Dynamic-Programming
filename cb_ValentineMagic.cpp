...
// https://hack.codingblocks.com/app/contests/1338/281/problem

int solve(vi &b, vi &g, vvi &dp, int i, int j){
    if(i==b.size()) return 0;
    if(j==g.size()) return 1e9;
    if(dp[i][j] != -1) return dp[i][j];
    int op1 = abs(b[i]-g[j]) + solve(b, g, dp, i+1, j+1);
    int op2 = solve(b, g, dp, i, j+1);
