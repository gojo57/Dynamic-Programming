...
int solve(int n, int cap, vi &size, vi &value, vvi &dp){
    if(n == 0 || cap == 0) return 0;
    if(dp[n][cap] != -1) return dp[n][cap];
    if(size[n-1] <= cap){
        return dp[n][cap] = max(value[n-1] + solve(n, cap-size[n-1], size, value, dp), solve(n-1, cap, size, value, dp));
    }
    else{
        return dp[n][cap] = solve(n-1, cap, size, value, dp);
    }
}
...
signed main()
{
    ...
    vvi dp(n+1, vi(cap+1, -1));
    cout<<solve(n, cap, size, value, dp)<<endl;
