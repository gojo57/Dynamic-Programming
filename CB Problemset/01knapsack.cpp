...
int solve(int n, int s, vi &size, vi &value, vvi &dp){
    if(n==0 || s==0) return 0;
    if(dp[n][s] != -1) return dp[n][s];
    if(size[n-1] <= s){
        return dp[n][s] = max(value[n-1] + solve(n-1, s-size[n-1], size, value, dp), solve(n-1, s, size, value, dp));
    }
    else{
        return dp[n][s] = solve(n-1, s, size, value, dp);
    }
}
