...
int solve(vvi &colors, int i, int m, vi &weight, int sum, vvi &dp, int x){
    if(i == m) return x-sum;
    if(dp[i][sum] != -1) return dp[i][sum];
    int ans = INT_MAX;
    for(int j=0;j<colors[i].size();j++){
        if(x >= weight[colors[i][j]]+sum){
            ans = min(ans, solve(colors, i+1, m, weight, weight[colors[i][j]]+sum, dp, x));
        }
    }
    return dp[i][x] = ans;
}
...
signed main()
{
    ...
    vvi colors(m);
    for(int i=0;i<n;i++) colors[color[i]-1].pb(weight[i]);
    for(int i=0;i<m;i++) sort(all(colors[i]));
    vvi dp(m+1, vi(x+1, -1));
    int ans = solve(colors, 0, m, weight, 0, dp, x);
