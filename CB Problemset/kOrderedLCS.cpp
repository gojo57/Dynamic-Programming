...
int solve(vi &v, vi &w, int i, int j, int k, vector<vvi> &dp){
    if(i < 0 || j < 0) return 0;
    if(dp[i][j][k] != -1) return dp[i][j][k];
    int ans = 0;
    if(v[i] == w[j]) ans = 1 + solve(v, w, i-1, j-1, k, dp);
    else{
        if(k > 0) ans = 1 + solve(v, w, i-1, j-1, k-1, dp);
        ans = max(ans, solve(v, w, i-1, j, k, dp));
        ans = max(ans, solve(v, w, i, j-1, k, dp));
    }
    return dp[i][j][k] = ans;
}
...
signed main()
{
    ...
    vector<vvi> dp(n+1, vvi(m+1, vi(k+1, -1)));
    cout<<solve(v, w, n-1, m-1, k, dp)<<"\n";
